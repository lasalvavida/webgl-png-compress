...
export default function compress (image, canvas2d, gl, n, iterations) {
  ...
  var positions = new Float32Array(imageWidth * imageHeight * 2)
  for (x = 0; x < imageWidth; x++) {
    for (y = 0; y < imageHeight; y++) {
      positions[y * imageWidth * 2 + x * 2] = (x / imageWidth - 0.5) * 2
      positions[y * imageWidth * 2 + x * 2 + 1] = -(y / imageHeight - 0.5) * 2
    }
  }
  ...
  var clusterCenters = createInitialClusterCenters(n, imageData, imageWidth, imageHeight)
  var pixelData = new Uint8Array(imageWidth * imageHeight * 4)
  ...
  for (var iteration = 0; iteration < iterations; iteration++) {
    ...
    gl.drawArrays(gl.POINT, 0, imageWidth * imageHeight)
    gl.readPixels(0, 0, imageWidth, imageHeight, gl.RGBA, gl.UNSIGNED_BYTE, pixelData)

    for (x = 0; x < imageWidth - 1; x++) {
      for (y = 0; y < imageHeight - 1; y++) {
        var index = y * imageWidth * 4 + x * 4
        var mirrorIndex = (imageHeight - y - 1) * imageWidth * 4 + x * 4
